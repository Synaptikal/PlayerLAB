...
class SleeperAPI {
  ...
  async getLeagueMatchups(leagueId: string, week?: number) {
    try {
      ...
      const response = await fetch(`${SLEEPER_BASE_URL}/league/${leagueId}/matchups${weekParam}`, {
        headers: {
          Accept: "application/json",
          "User-Agent": "PlayerLAB/1.0",
        },
  ...
  async getPlayerStats(season: string, week?: number) {
    try {
      const weekParam = week ? `&week=${week}` : ''
      const response = await fetch(`${SLEEPER_BASE_URL}/stats/nfl/${season}?season_type=regular${weekParam}`, {
        headers: {
          Accept: "application/json",
          "User-Agent": "PlayerLAB/1.0",
        },
  ...
  async getPlayerProjections(season: string, week?: number) {
    try {
      const weekParam = week ? `&week=${week}` : ''
      const response = await fetch(`${SLEEPER_BASE_URL}/projections/nfl/${season}?season_type=regular${weekParam}`, {
        headers: {
          Accept: "application/json",
          "User-Agent": "PlayerLAB/1.0",
        },
  ...
  async getWaiverWire(leagueId: string) {
    try {
      ...
      return transactions.filter((t: any) => t.type === 'add' || t.type === 'drop')
    } catch (error) {
  ...
  async getTrades(leagueId: string) {
    try {
      ...
      const transactions = await response.json()
      return transactions.filter((t: any) => t.type === 'trade')
    } catch (error) {
